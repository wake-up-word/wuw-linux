...
// mix_n =   |means|vars|
//-----------------------------------------------------------------------------
bool HmmUtil::SaveBinaryHmm(HmmModel* m, char* filename)
{
...
// mix_n =   |means|vars|
//-----------------------------------------------------------------------------
HmmModel* HmmUtil::LoadBinaryHmm(char* filename)
{
	...
	HmmState* t = new HmmState[nStates];
	m->states = t;
	...
	int size = nStates + 2;
	m->transProb = new FLOAT64*[size];

	for(int i = 0; i < size; i++)
	{
		double* test = new FLOAT64[size];
		m->transProb[i] = test;
		fread(m->transProb[i], sizeof(FLOAT64), size, f);
	}
