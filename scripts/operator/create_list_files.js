...
module.exports = {
    run: () => {
        ...
        const hmmTrainingOperatorFiles = operatorFiles.map(f => path.resolve(fefTrainingOutDir, `${f.fileName.replace('.', '_')}.fef`));
        fs.writeFileSync(hmmTrainingOperatorListFile, hmmTrainingOperatorFiles.join('\n'));
        const hmmTrainingOutOfVocabFiles = outOfVocabFiles.map(f => path.resolve(fefTrainingOutDir, `${f.fileName.replace('.', '_')}.fef`));
